...
For *speed* on PSMP-fronted bastions we recommend keeping
~RemoteCommand sudo -i~ in the bastion's ssh_config stanza: the
elevation then happens during the SSH handshake instead of costing
an extra interactive round-trip after login.  The auto-stepper
(see `emacs.d/vterm--advance-pending-chain') is idempotent: it
notices when the queued `sudo -i -u root' step is already
satisfied by the arriving session (because RemoteCommand has
elevated us) and silently drops it, so the two layers compose
cleanly.

The cost: ~RemoteCommand~ confuses TRAMP's prompt detection on the
*same* alias.  We therefore declare two Host stanzas per bastion --
an *interactive* alias that carries the RemoteCommand for vterm
sessions, and a *clean* TRAMP-only alias pointing at the same
HostName -- and point TRAMP at the latter via the ~:tramp-alias~
key of `emacs.d/ssh-host-tramp-config'.  Example:

#+begin_src conf-space :tangle no
# Interactive: fast PSMP-friendly auto-elevation via RemoteCommand
Host lxsag811 lxsag011 lxsag012
...
# TRAMP-only: no RemoteCommand, no RequestTTY -- clean for TRAMP
Host lxsag811-tramp lxsag011-tramp lxsag012-tramp
    HostName     psmpcyb.gbm.lan
    User         matteo.aquino@CYB-ADMIN1@%n.gbm.lan
    IdentityFile ~/.ssh/id_acme
    PreferredAuthentications publickey,keyboard-interactive
...
If you cannot keep ~RemoteCommand~ in your ssh_config (e.g. corporate
policy forbids it, or you prefer a single Host stanza), simply omit
it: the stepper falls back to typing ~sudo -i -u root~ interactively
on the first bastion prompt -- correct, just one round-trip slower.
...
Two bastions chosen by the host-name *digit group* are the
defaults baked into ~emacs.d/ssh-target-bastions~: the first digit
after the alpha prefix selects the data-centre / row, so
`lxumc811' belongs to the 8xx world routed through `lxsag811',
whereas `lxohr002' is in the 0xx world routed through `lxsag011'.
No manual ~setq~ needed -- the defaults already match this user's
environment.

#+begin_src emacs-lisp :tangle no
;; Built-in defaults (see the defcustom in setup-terminal.el and
;; setup-ssh-sessions.el).  Override only when working from a
;; machine with a different bastion topology.
emacs.d/ssh-host-tramp-config
  ;; => '(("\\`lxsag" :sudo-user "root"))
...
So ~C-c s s oracle@lxumc811~ opens a vterm running
=ssh -t lxsag811 sudo -i -u root ssh -t lxumc811 sudo -i -u oracle=
straight away, ~C-c s s root@lxohr002~ uses =lxsag011= instead,
and `C-c f' / `C-c d' inside each vterm reach the same file-system
view via TRAMP.
