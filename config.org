...
detection (and also collide with the multi-step chain this module
appends to the ssh CLI -- OpenSSH refuses with ~Cannot execute
command-line and remote command~), so we move all the "auto-elevate
after login" logic to Emacs.  A typical bastion entry looks like:
...
The shell command the module synthesises is one line -- e.g. for
~C-c s s oracle@lxumc811~:

#+begin_src text :tangle no
ssh -t lxsag811 sudo -i -u root ssh -t lxumc811 sudo -i -u oracle
#+end_src

PSMP receives the whole pipeline as a single SSH exec request and
hands the inner shell commands through to the bastion, where they
execute in sequence.  Elevation is faster than a typed-by-hand
flow because there is no round-trip waiting for each prompt to
appear.
