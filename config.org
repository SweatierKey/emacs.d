...
For bastion entries we recommend a *clean* ssh_config stanza -- no
~RemoteCommand~, no ~RequestTTY~ -- because TRAMP's prompt detection
hates them.  For vterm sessions specifically, the multi-step chain
this module synthesises in shell would also collide with a
~RemoteCommand~ inherited from the config (OpenSSH refuses with
~Cannot execute command-line and remote command~) -- but the bastion
hop is built with ~-o RemoteCommand=none~ exactly to neutralise that
case, so leaving ~RemoteCommand sudo -i~ in your config is no longer
fatal.  A typical bastion entry looks like:
